...
def start_exam():
    ...
    # Select 15 random questions. The FULL objects (including the correct
    # 'answer') are kept server-side only, in the session, so /submit can
    # score against them later.
    selected_questions = random.sample(questions, 15)
    for i, q in enumerate(selected_questions):
        q['index'] = i  # Add unique index to each question
    session['questions'] = selected_questions

    # Build a CLIENT-SAFE copy for rendering. The 'answer' field is
    # deliberately left out here - it must never reach the browser, whether
    # via the rendered HTML or by being handed to the template context at
    # all (a future template edit could otherwise leak it again).
    display_questions = [
        {
            'index': q['index'],
            'question': q['question'],
...
def submit_exam():
    try:
        ...
        # Calculate score - always against the server-side session copy,
        # never against anything submitted by the client.
        score = 0
